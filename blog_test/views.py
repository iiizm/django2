...
def analysis(request):
    if request.method == "POST":
        form = crawling_info(request.POST)
        ck_freq_wc = request.POST.getlist('ck_freq_wc')
        if request.POST and request.FILES:
            ...
            for n, c in count.most_common(30):
                df.loc[len(df)] = [n, c]
            my_dpi = 96
            fig = plt.figure(figsize=(6.2, 6.2), dpi=100)
            ax = fig.add_subplot(111)
            plt.bar(range(0, len(df)), df['freq'])
            plt.xticks(range(0, len(df)), df['word'])
            # plt.gca().invert_yaxis()
            plt.xticks(rotation=90, ha='center')
            #plt.margins(x=0, y=0)
            # plt.tick_params()
            # df.plot(kind='bar', x='word', y='freq', color='grey', legend=None)
            # fig_html_freq = mpld3.fig_to_html(fig)
            buf = BytesIO()
            fig.savefig(buf, format='png')
            img_freq = base64.b64encode(buf.getvalue()).decode('utf-8').replace('\n', '')
            buf.close()
            html_table = df.to_html(index=False)

            #wordcloud
            fig2 = plt.figure(figsize=(6.2, 6.2), dpi=100)
            ax2 = fig2.add_subplot(111)
            voca = nltk.Text(nouns)
            data = voca.vocab().most_common(50)
            tmp_data = dict(data)
            wc = wordcloud.WordCloud(font_path="/usr/share/fonts/truetype/nanum/NanumGothic.ttf", relative_scaling=0.2, width=620, height=620,
                                     background_color="white", margin=10).generate_from_frequencies(tmp_data)
            ax2.imshow(wc, interpolation='bilinear')
            ax2.axis('off')
            ax2.margins(x=0, y=0)
            buf_wc = BytesIO()
            fig2.savefig(buf_wc, format='png')
            img_wc = base64.b64encode(buf_wc.getvalue()).decode('utf-8').replace('\n', '')
            buf_wc.close()
            if 'freq' in ck_freq_wc and 'wc' in ck_freq_wc:
                form = crawling_info()
                return render(request, 'blog_test/analysis.html', {'form': form, 'img_freq': img_freq, 'img_wc': img_wc})
            elif 'freq' in ck_freq_wc:
                form = crawling_info()
                return render(request, 'blog_test/analysis.html', {'form': form, 'img_freq': img_freq})
            elif 'wc' in ck_freq_wc:
                form = crawling_info()
                return render(request, 'blog_test/analysis.html', {'form': form, 'img_wc': img_wc})
    else:
        form = crawling_info()
        return render(request, 'blog_test/analysis.html', locals())
...
def file_input2(request):
    ...
    if request.POST and request.FILES:
        ...
        data = voca.vocab().most_common(50)
        tmp_data = dict(data)
        wc = wordcloud.WordCloud(font_path="c:/Windows/Fonts/malgun.ttf", relative_scaling=0.2, height=600, width=600,
                                 background_color="white", margin=10).generate_from_frequencies(tmp_data)
        ax2.imshow(wc, interpolation='bilinear')
        ax2.axis('off')
        ax2.margins(x=0, y=0)
        ...
        return render(request, 'blog_test/file_input2.html', {'img_freq': img_freq, 'img_wc': img_wc, 'html_table': html_table})
    # list = ['평창', '동계', '올림픽']
